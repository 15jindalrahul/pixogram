...
export class UserService {
  private baseUrl = 'http://localhost:9021/user';

  private followUrl='http://localhost:9023/follow'
  ...
  getFollowing(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:9023/follow/following/`+`${userid}`)
  }

  getFollower(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:9023/follow/follower/`+`${userid}`)
  }
